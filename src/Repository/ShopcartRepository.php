...
class ShopcartRepository extends ServiceEntityRepository
{
    ...
    public function total($id):float{

        $em = $this->getEntityManager();
        $query = $em->createQuery('SELECT sum(p.sprice * s.quantity) as total
        FROM App\Entity\Shopcart s, App\Entity\Admin\Product p WHERE s.productid = p.id and s.userid=:id')->setParameter('id', $id);
    ...
    public function products($id):array{
        ...
        $em = $this->getEntityManager();
        $query = $em->createQuery('SELECT p.title,p.sprice,  s.id,s.quantity,s.productid,s.userid, (p.sprice * s.quantity) as total
